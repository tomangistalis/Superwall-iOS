...
>: View {
  ...
  /// A custom loading `View`. This will show while the requested paywall request  is loading.
  private var onLoadingView: OnLoadingView
  ...
  ///   - onLoadingView: An optional view builder that returns a custom loading `View`.  This will show while the requested paywall request
  ///   is loading. Defaults to `EmptyView()`.
  ...
  public init(
    ...
    @ViewBuilder onLoadingView: () -> OnLoadingView = EmptyView.init,
    @ViewBuilder onSkippedView: @escaping (PaywallSkippedReason) -> OnSkippedView = { _ in EmptyView() },
    @ViewBuilder onErrorView: @escaping (Error) -> OnErrorView = { _ in EmptyView() },
    feature: (() -> Void)? = nil
  ) {
